...
$name = $email = $phone = $message = $pet = $mainkey = $copy = $success = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($name_error == '' and $email_error == '' and $phone_error == ''){
      ...
      $to = 'd1thulin@live.se';
      $subject = 'Felanmälan';
      $subjectback = 'Kopia på felanmälan';
      if (mail($to, $subject, $message)){
          if($copy == 'yes'){
          mail($email,$subjectback, $message);
          }
          $success = "Meddelande skickat, tack för att du kontaktade oss!";
          $name = $email = $phone = $message = $pet = $mainkey = $copy = '';
      }
  }
